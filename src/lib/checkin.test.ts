...
describe("check-in is the room", () => {
  it("matches two people on the same shop id", () => {
    ...
    expect(presenceMatchSource(them, { shopId: "wizards-loft-red-deer", tableOn: false, tableCode: "AB12" })).toBe(
      "shop",
    );
  ...
  it("does not treat GPS-only presence as in the room", () => {
    ...
    expect(presenceMatchSource(them, { shopId: "wizards-loft-red-deer", tableOn: false, tableCode: "AB12" })).toBeNull();
  ...
  it("still allows an optional table code as a side channel", () => {
    const them = peer({ room: "AB12" });
    expect(presenceMatchSource(them, { tableOn: true, tableCode: "AB12" })).toBe("table");
    expect(presenceMatchSource(them, { shopId: "wizards-loft-red-deer", tableOn: false, tableCode: "AB12" })).toBeNull();
  });
});
